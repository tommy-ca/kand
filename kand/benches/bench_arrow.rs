...
use kand::{TAFloat, TAInt, ta::ohlcv::{sma, macd, cdl_hammer}};
...
fn bench_macd_comparison(c: &mut Criterion) {
    ...
    for &size in &data_sizes {
        ...
        group.bench_with_input(BenchmarkId::new("Raw_Slice", size), &size, |b, _| {
            b.iter(|| {
                macd::macd_raw(&input, 12, 26, 9, &mut output_macd, &mut output_signal, &mut output_hist);
            });
        });
...
fn bench_hammer_comparison(c: &mut Criterion) {
    ...
    for &size in &data_sizes {
        ...
        group.bench_with_input(BenchmarkId::new("Raw_Slice", size), &size, |b, _| {
            b.iter(|| {
                cdl_hammer::cdl_hammer_raw(&open, &high, &low, &close, 14, 2.0, &mut output_signals, &mut output_body_avg);
            });
        });
        ...
        group.bench_with_input(BenchmarkId::new("Arrow_Pooled", size), &size, |b, _| {
            b.iter(|| {
                let _ = cdl_hammer::cdl_hammer_arrow(&open_arrow, &high_arrow, &low_arrow, &close_arrow, 14, 2.0).unwrap();
            });
        });
    }
    group.finish();
...
criterion_group!(benches, bench_sma_comparison, bench_macd_comparison, bench_hammer_comparison);
